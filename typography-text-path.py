...
def contour(X, Y, text, offset=0):
    ...
    dX, dY = dX / (norm + np.finfo(float).eps), dY / (norm + np.finfo(float).eps)    
    X0 += -V[:, 1] * dY
    Y0 += +V[:, 1] * dX
    V[:, 0], V[:, 1] = X0, Y0

    modified_path = Path(V, path.codes)
    ...
    # Actual text
    patch = PathPatch(
        modified_path, facecolor="black", zorder=30, edgecolor="black", linewidth=0.0
    )
    ax.add_artist(patch)
